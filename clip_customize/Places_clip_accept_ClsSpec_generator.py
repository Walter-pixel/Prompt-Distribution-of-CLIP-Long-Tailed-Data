...
class Prompt_Generator_Wrapper(nn.Module):
    def __init__(self, cfg, classnames, clip_model, latent_code_dim):
        ...
        self.mn_std_net = nn.Sequential(OrderedDict([
            ('layer0', nn.Linear(latent_code_dim * self.n_ctx, 200, dtype=self.dtype)),
            ('act0', nn.LeakyReLU()),
    ...
    def forward(self):
        h_e = self.mn_std_net(torch.flatten(self.latent_code, start_dim=1))  # will not update self.conditioned as it's registered in buffer
