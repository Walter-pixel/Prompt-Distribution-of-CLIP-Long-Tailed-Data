...
import time
from clip_customize.Places_clip_accept_ClsAgn_generator import*
import yaml
...
(0) promput generator outputs the mu & std at the end <not write to self.mu_accumulate/self.std_accumulate, so this is memery consuming>
    --> this is to fit into multiple gpus training, if you have A40, then do remember to modify them back
(1) This script only optimize the prompt generator part
(2) The generator is class agnostic generator, NO class label is fed to the generator <so not conditioned>
(3) this script uses python file: "Places_clip_customize/clip_accept_ClsAgn_generator.py"
...
def main(args):
    ...
    for epoch in range(e_start, args.epochs):
        ...
        train(loaders['train'], epoch, args, info, imb_clip_model, optim_gen, all_losses)
...
def train(loader, epoch, args, info, imb_clip_model, optim_gen, all_losses):
    # since reparameterization like VAE, ony sample once in the forward pass
    ...
    for step, (images, labels, _) in enumerate(loader):
        ...
        eval(md).prompt_learner.latent_code.grad.data.zero_()
    ...
    # # add mn_std to histogram <plot all entries of mu & std for all prompt matrix of all classes>:
    # info.writer.add_histogram("Train/Mean", mu_batch/bz, bins=200, global_step=epoch)
    # info.writer.add_histogram("Train/STD", std_batch/bz, bins=200, global_step=epoch)


    if epoch % 5 ==0 or epoch >= args.epochs-3:
...
def test_multisample(test_loader, train_loader, epoch, args, info, imb_clip_model):
    ...
    for step, (images, labels, _) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            total_pred = torch.cat((total_pred,preds.to('cpu')), 0)
            # append to mu & std container to record
    ...
    # # add mn_std to histogram <plot all entries of mu & std for all prompt matrix of all classes>:
    # info.writer.add_histogram("Test/Mean", mu_batch / bz, bins=200, global_step=epoch)
    # info.writer.add_histogram("Test/STD", std_batch / bz, bins=200, global_step=epoch)
...
class CLIP_Lrn_PromptG(nn.Module):
    ...
    def forward(self, image, get_feature_only=False, n_smaple=1, combine_mode='None'):
        ...
        if combine_mode == 'avg':
            ...
            # txt_cache = torch.zeros(self.n_classes, n_smaple, image_features.size(dim=-1), dtype=self.dtype).to(image_features.device) # dim=[n_class, n_sample, emb-len]
            txt_cache = torch.zeros(self.n_classes, image_features.size(dim=-1), dtype=self.dtype).to(image_features.device)

            ctx_matrix, log_var_matrix, kl_loss = self.prompt_learner()
            tokenized_prompts = self.tokenized_prompts
            for n in range(n_smaple): # modify 12 Oct <ut prompt sampling within the for loop>
                #each time ctx_martrix is given to all classes
                std_matrix = torch.exp(0.5 * log_var_matrix)
                eps_matrix = torch.randn_like(std_matrix)
                ctx_matrix = ctx_matrix + eps_matrix * std_matrix # elementwise product
                ctx_matrix_share = ctx_matrix.unsqueeze(0).repeat(self.n_classes, 1, 1)
                prompts_to_txtenc = self.prompt_learner.build_txt_enc_input(ctx_matrix_share)
                text_features = self.text_encoder(prompts_to_txtenc.type(self.dtype), tokenized_prompts.type(self.dtype))
            ...
            # if combine_mode == 'avg': # memory hungry
            #     # take average on text encoder output of the same class first before computing loss with image_feature
            #     avg_text_features = torch.mean(txt_cache, dim=1, keepdim=False)
            #     logit_scale = self.logit_scale.exp()
            #     logits = logit_scale * image_features @ avg_text_features.t()
            #     return logits, kl_loss_sum #[bz, n_classes], scalar
            #
            #
            ...
            # elif combine_mode == 'max_within_cls':
            #     # each logit element is the highest similarity score of image_feature w.r.t. multiple text encoder output of that class
            #     bz, feat_len = image_features.size()
            #     # Now trasnpose img_feat, each column is a sample from the batch
            #     # --> then duplicate "n_classes" # of such matrix in order to matrix multiply with "n_sample" # of text_features per-class
            #     img_enc_transpose_expand = image_features.clone().t().unsqueeze(dim=0).expand(self.n_classes, -1, -1) # dim -> [num_classes, feat_len, bz_of_ImgEnc]
            #     all_scores = torch.matmul(txt_cache, img_enc_transpose_expand)  # dim=[num_classes, n_sample, bz_of_ImgEnc]
            #     max_score_percls = torch.amax(all_scores, dim=1) # dim-> [n_classes, bz]
            #     max_score_percls = max_score_percls.t()
            #
            #     return max_score_percls, kl_loss, mu, std #[bz, n_classes], scalar, , ,
